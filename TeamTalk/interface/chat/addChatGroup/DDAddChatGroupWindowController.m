...
- (void)p_createGroupFinished:(GroupEntity*)group;
- (void)p_addMemberInGroupFinished:(GroupEntity*)group;
...
- (NSView*)tableView:(NSTableView *)tableView viewForTableColumn:(NSTableColumn *)tableColumn row:(NSInteger)row
{
    ...
    if ([identifier isEqualToString:@"selectedColumn"])
    {
        ...
        UserEntity* user = self.model.selectUsers[row];
...
- (IBAction)pressOKBtnOnAddChatGroup:(id)sender {
    ...
    if (_addType==0) {
        ...
        for (UserEntity* user in self.model.selectUsers)
        {
            [userIdList addObject:user.userId];
        }
        
        DDAddMemberToGroupAPI* addMemberToGroupAPI = [[DDAddMemberToGroupAPI alloc] init];
...
- (IBAction)addGroupNameOk:(id)sender {
    ...
    if (_addType==0) {      //创建
        //创建的时候,加上自己,加当
        DDUserlistModule *userIdListModule =[DDUserlistModule shareInstance];
        UserEntity *myUser = [userIdListModule myUser];
        [userIdList addObject:myUser.userId];
...
- (void)p_createGroupFinished:(GroupEntity*)group
{
    if (group)
    {
        group.groupUpdated = [[NSDate date] timeIntervalSince1970];
        group.groupCreatorId = [[DDUserlistModule shareInstance] myUserId];
        ...
        [sessionModule createGroupSession:group.groupId type:GROUP_TYPE_TEMPORARY];
        [NotificationHelp postNotification:notificationReloadTheRecentContacts userInfo:nil object:group.groupId];
    }
}
