...
- (BOOL)performDragOperation:(id<NSDraggingInfo>)sender
{
    if ( [sender draggingSource] != self ) {
        ...
        if([NSImage canInitWithPasteboard: [sender draggingPasteboard]]) {
            ...
            [self addAttachmentOfImage:newImage];
            return YES;
        }
        //if the drag comes from a file, set the window title to the filename
        else if ([NSURL URLFromPasteboard: [sender draggingPasteboard]])
        {
            NSURL* fileURL=[NSURL URLFromPasteboard: [sender draggingPasteboard]];
            NSString* file = [fileURL path];
            DDLog(@"drag in file: %@, toUserId: %@", file, sessonEntity.orginId);
            if ([file hasSuffix:@"png"] || [file hasSuffix:@"jpg"] || [file hasSuffix:@"jpeg"] || [file hasSuffix:@"gif"] || [file hasSuffix:@"bmp"])
            {
                NSAttributedString* attribute = [NSAttributedString imageAttributedString:file realImageURL:nil compressImage:NO];
            ...
            else
            {
                ...
                // filter file transfer in offline state
                if ([[StateMaintenanceManager instance] getMyOnlineState] == USER_STATUS_OFFLINE) {
                    return NO;
                }
                NSString* fromUserID = [DDClientState shareInstance].userID;
                DDFileEntity* fileEntity = [[DDFileEntity alloc] initWithType:SendFileType taskID:nil fromUserID:fromUserID toUserID:sessonEntity.sessionId filePath:file fileName:[file lastPathComponent] fileSize:0 ips:nil port:0];
                [[DDFileModule shareInstance] sendFileEntity:fileEntity];
            }
        }
        else
        {
