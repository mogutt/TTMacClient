...
- (IBAction)onSettingChanged:(id)sender {
    ...
    /*
     if (rdi.msgSendKey == rdx) {
     r15 = *objc_msgSend;
     rax = [rdx indexOfSelectedItem];
     [*_RTXInfo setIsEnter:(rax == 0x0 ? 0xff : 0x0) & 0xff];
     }
     else {
     if (rbx.otherDownlaodFileLocMenuItem == rdx) {
     [rbx setDownloadDefaultDir];
     }
     }
     rdi = rbx;
     rax = [rdi saveGeneralSetting];
     return rax;
