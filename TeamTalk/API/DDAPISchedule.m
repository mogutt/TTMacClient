...
- (void)receiveServerData:(NSData*)data forDataType:(ServerDataType)dataType
{
    dispatch_async(self.apiScheduleQueue, ^{
        ...
        if(dataType.serviceID == 6 && dataType.commandID == 17)
        {
            DDLog(@"asd");
        }
...
{
    [[DDTcpClientManager shareInstance] writeData:data];
}

- (void)sendDataToFileClient:(NSData*)data
{
    [[DDFileTcpClient shareInstance] writeData:data];
}
