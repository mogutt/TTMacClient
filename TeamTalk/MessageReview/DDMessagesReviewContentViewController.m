...
- (void)updateSession:(NSString*)sessionID
{
    ...
    [[DDDatabaseUtil instance] getMessagesCountForSessionID:sessionID completion:^(NSInteger count) {
        NSInteger pagecount = count % 20 == 0 ? count / 20 : count / 20 + 1;
...
- (void)p_addNameAndTimeTextField:(MessageEntity *)message
{
    ...
    {
        //是登录用户发的
        NSString* name = [userModule myUser].name;
    ...
    {
        //是别人发的
        UserEntity* sender = [userModule getUserById:senderID];
